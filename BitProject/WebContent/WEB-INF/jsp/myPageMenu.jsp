<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<html>


	<h2><a href="memberInfo" target="myPageContent">My Page</a></h2>
	<h3><a href="memberInfo" target="myPageContent">내 정보</a></h3>
	<div id=""><h3><a href="" target="myPageContent">판매자 등록</a></h3></div>
	<div id="sellerMyPage" style="display:none;"><h3><a href="sellerMyPage" target="myPageContent">내 상점</a></h3></div>
	<h3><a href="followingNotiMain" target="myPageContent">관심스토어 알림</a></h3>
	<h3><a href="productDibs" target="myPageContent">찜한 상품</a></h3>
	<h3><a href="productBasket" target="myPageContent">장바구니</a></h3>
	<h3><a href="consumerContractList" target="myPageContent">주문/배송</a></h3>
	<h3><a href="qAndAMain" target="myPageContent">문의사항</a></h3>
	<h3><a href="costomerCenter" target="myPageContent">고객센터</a></h3>
	<h3><a href="deleteMember" target="myPageContent">탈퇴하기</a></h3>


</html>